    ...
    <main className="min-h-screen bg-black text-white">
      ...
      <section className="max-w-6xl mx-auto px-4 py-8 md:py-10">
        ...
        >
          ...
          {formData && (
            ...
            >
              ...
              <div className="bg-white/[0.015] border border-primary-400/15 rounded-xl p-2.5 md:p-3 lg:p-4 text-center">
                <div className="text-[10px] md:text-xs text-white/50 uppercase tracking-wider mb-0.5 md:mb-1">Experience</div>
                <div className="text-xs md:text-sm lg:text-lg font-semibold">{formData.experience}</div>
              </div>
            </motion.div>
          )}
        </motion.div>
        ...
        {parsedData ? (
          <>
              ...
              >
                ...
                <div className="space-y-6 md:space-y-10">
                  {parsedData.days.map((day, idx) => (
                    ...
                    >
                      <div className="px-4 md:px-6 py-3.5 md:py-5 border-b border-white/5 bg-white/[0.02]">
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-1.5 md:gap-2">
                          <div>
                            <div className="text-[10px] md:text-xs text-primary-400 font-black uppercase tracking-[0.22em] mb-0.5 md:mb-1">
                              {day.day}
                            </div>
                          ...
                          {day.targetMuscles && (
                            <div className="flex items-center gap-1.5 md:gap-2 text-white/60 text-xs md:text-sm lg:text-base">
                              <Dumbbell size={14} className="md:w-[16px] md:h-[16px] text-primary-400/70" />
                        ...
                        <table className="w-full text-left">
                          <thead>
                            <tr className="bg-primary-400/10 border-y border-primary-400/20">
                              <th className="py-2 md:py-3 lg:py-4 px-2.5 md:px-3 lg:px-6 text-[10px] md:text-xs lg:text-sm font-bold text-primary-400 uppercase tracking-wider w-2/5 md:w-auto">
                                Exercise
                              </th>
                              <th className="py-2 md:py-3 lg:py-4 px-1.5 md:px-2 lg:px-4 text-[10px] md:text-xs lg:text-sm font-bold text-primary-400 uppercase tracking-wider w-auto md:w-1/6 text-center">
                                Sets
                              </th>
                              <th className="py-2 md:py-3 lg:py-4 px-1.5 md:px-2 lg:px-4 text-[10px] md:text-xs lg:text-sm font-bold text-primary-400 uppercase tracking-wider w-auto md:w-1/6 text-center">
                                Reps
                          ...
                          <tbody>
                            {day.exercises.map((exercise, eIdx) => (
                              ...
                              >
                                <td className="py-2 md:py-2.5 lg:py-4 px-2.5 md:px-3 lg:px-6 font-medium text-xs md:text-sm lg:text-base break-words">
                                  {exercise.name}
                                </td>
                                ...
                                <td className="py-2 md:py-2.5 lg:py-4 px-1.5 md:px-2 lg:px-4 text-center text-white/85 text-xs md:text-sm lg:text-base">
                                  {exercise.weight || estimateWeight(exercise.name, formData)}
                                </td>
                                <td className="py-2 md:py-2.5 lg:py-4 px-1.5 md:px-2 lg:px-4 text-center text-white/85 text-xs md:text-sm lg:text-base">
                                  {exercise.rest}
                                </td>
            ...
            <div className="grid md:grid-cols-2 gap-4 md:gap-6 mb-10 md:mb-12">
              <motion.div
                ...
                className="bg-white/[0.015] border border-primary-400/20 rounded-2xl p-4 md:p-6 backdrop-blur-sm"
              >
                <div className="flex items-center gap-2 md:gap-3 mb-3.5 md:mb-5">
                  <div className="w-9 md:w-12 h-9 md:h-12 bg-primary-400/10 rounded-xl flex items-center justify-center">
                    <Flame size={20} className="md:w-[24px] md:h-[24px] text-primary-400" />
                  </div>
                  <h3 className="text-lg md:text-xl font-bold">Warm Up</h3>
                </div>
                <ul className="space-y-2 md:space-y-3">
                  {parsedData.warmUp.length > 0 ? (
                    parsedData.warmUp.map((item, idx) => (
                      <li key={idx} className="flex items-start gap-2 md:gap-3">
                        <div className="w-4 md:w-5 h-4 md:h-5 rounded-full border border-primary-400/50 mt-0.5 md:mt-0.5 flex-shrink-0" />
                        <span className="text-white/70 text-xs md:text-sm">{item}</span>
                      </li>
                    ))
                  ) : (
                    <li className="text-white/50 text-xs md:text-sm">5-10 minutes of light cardio + dynamic stretching</li>
                  )}
                </ul>
              </motion.div>
              ...
              >
                <div className="flex items-center gap-2 md:gap-3 mb-3.5 md:mb-5">
                  <div className="w-9 md:w-12 h-9 md:h-12 bg-primary-400/10 rounded-xl flex items-center justify-center">
                    <TrendingUp size={20} className="md:w-[24px] md:h-[24px] text-primary-400" />
                  </div>
                  <h3 className="text-lg md:text-xl font-bold">Progression Tips</h3>
                </div>
                <ul className="space-y-2 md:space-y-3">
                  {parsedData.tips.length > 0 ? (
                    parsedData.tips.map((item, idx) => (
                      <li key={idx} className="flex items-start gap-2 md:gap-3">
                        <div className="w-4 md:w-5 h-4 md:h-5 rounded-full border border-primary-400 flex items-center justify-center flex-shrink-0 mt-0.5 md:mt-0.5">
                          <div className="w-1.5 md:w-2 h-1.5 md:h-2 bg-primary-400 rounded-full" />
                        </div>
                        <span className="text-white/70 text-xs md:text-sm">{item}</span>
                      </li>
                    ))
                    ...
                    <>
                      <li className="flex items-start gap-2 md:gap-3">
                        ...
                        <span className="text-white/70 text-xs md:text-sm">Increase weight gradually when you hit top reps</span>
                      </li>
                      <li className="flex items-start gap-2 md:gap-3">
                        <div className="w-4 md:w-5 h-4 md:h-5 rounded-full border border-primary-400 flex items-center justify-center flex-shrink-0 mt-0.5 md:mt-0.5">
                          <div className="w-1.5 md:w-2 h-1.5 md:h-2 bg-primary-400 rounded-full" />
                        </div>
                        <span className="text-white/70 text-xs md:text-sm">Focus on form over weight</span>
                      </li>
                    </>
                  )}
                </ul>
            ...
            {parsedData.coolDown.length > 0 && (
              <motion.div
                ...
                className="mb-10 md:mb-12 bg-white/[0.015] border border-primary-400/15 rounded-2xl p-4 md:p-6 backdrop-blur-sm"
              >
                <div className="flex items-center gap-2 md:gap-3 mb-3.5 md:mb-5">
                  <div className="w-9 md:w-12 h-9 md:h-12 bg-primary-400/10 rounded-xl flex items-center justify-center">
                    <TrendingUp size={20} className="md:w-[24px] md:h-[24px] text-primary-400" />
                  </div>
                  <h3 className="text-lg md:text-xl font-bold">Cool Down</h3>
                </div>
                <ul className="space-y-2 md:space-y-3">
                  {parsedData.coolDown.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-2 md:gap-3">
                      <div className="w-4 md:w-5 h-4 md:h-5 rounded-full border border-primary-400/40 mt-0.5 md:mt-0.5 flex-shrink-0" />
                      <span className="text-white/70 text-xs md:text-sm">{item}</span>
                    </li>
                  ))}
                </ul>
          ...
          <div className="bg-white/[0.02] border border-primary-400/20 rounded-3xl p-6 md:p-10 text-center">
            <p className="text-white/60 text-sm md:text-base">No workout plan generated yet. Go back to generate one!</p>
          </div>
        )}
